...
int gy_flag=0;
int packit_count=0;
extern data_hand_t data_hand;
extern packet_hand_t packet_hand;
///////////////////////////////////////////////////////////////////////////
...
void computeIMU () {
  ...
  gy_total=imu.gyroADC[0]+imu.gyroADC[1]+imu.gyroADC[2];



  if(cl_flag==1){
       if(imu.gyroADC[0]<2000   && gy_total<2000)
          cl_flag=0; 
    }
  else if(cl_flag==2){
      if(imu.gyroADC[0]>-2000 && gy_total >-2000)
  ...
  //Page UP
  if(imu.gyroADC[0]>2000   && gy_total>2000 &&  cl_flag==0){
    data_hand.pg[0]++;
    cl_flag=1;
  }
  ...
  if(imu.gyroADC[0]<-2000 && gy_total<-2000 &&  cl_flag==0){
    ...
    cl_flag=2;
            ...
            packet_hand.start_byte      =   '$';
            packet_hand.second_byte      =   'M';
            packet_hand.direct_byte      =   '<';
            packet_hand.size_byte      =   sizeof(data_hand);
            packet_hand.command_byte   =   100;



            packet_hand.data = data_hand;
            packet_hand.crc_byte = 0;

            char *crc_cal   =   (char *)&(packet_hand.size_byte);
            while(crc_cal   !=   &packet_hand.crc_byte){
               packet_hand.crc_byte ^= *(crc_cal++);
            }
 
         Serial1.write(( uint8_t*)(&packet_hand),sizeof(packet_hand)); 
...
////////////////////////////////////////////////  CLICK  //////////////////////
  a_total=abs(imu.magADC[0])+abs(imu.magADC[1])+abs(imu.magADC[2]);
  ...
  if(a_total>=3000 && a_flag==0){
     data_hand.sw[0]=2;
     a_flag=1;
   }

 //Serial.print(a_flag); Serial.print("     ");Serial.print(a_total); Serial.print("sw[0] : "); Serial.println(data_hand.sw[0]); 
///////////////////////////////////////////////////////////////////////////
